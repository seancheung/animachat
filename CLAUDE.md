...
- **Messages hold variants (swipes)**: regeneration appends a variant; edits modify the active variant in place (no branching). Edits that touch summarized ranges must call `invalidateSummary`. Going back in time is a **fork at a message** (new chat copying the timeline up to it) — never destructive truncation.
