...
| **Chat** | Three modes — casual, immersive (fixed scene/location), **playthrough** (a self-contained snapshot run of a story, started from the Stories page where playthroughs are also listed; play as a cast member). In casual/immersive you can instead **play as the narrator** — you write the narration, the characters respond. Streaming with inline tag parsing, in-place edits (no branching), regenerate-as-swipes on the newest message (frozen to the pick once the chat moves on; branch from earlier points by forking), group chats with LLM turn orchestration + force-speaker, impersonate, per-chat language & POV (user-1st / all-3rd / VN-2nd) |
