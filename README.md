...
| **Entities** | Characters (sprites, custom expressions, image prompt), personas, locations & scenes (16:9 art, BGM, ambient loop, image prompt, per-place stage & chat-panel coloring with a global toggle), stories (ordered scenes), lorebooks (keyword-triggered) — all reusable, all import/exportable as zip bundles with assets |
