...
| **Narrator** | Optional (required in playthroughs, where it directs); speaks first; auto or summoned; 2–4 suggested actions as buttons; advances scenes via `<next-scene/>`, stages the cast via `<enter>`/`<leave>`, concludes via `<the-end/>` — all event-sourced from the timeline, so forks restore scene, presence, art and BGM |
...
Chat prose carries six inline tags, parsed out of the stream and stored as message metadata: `<emo>name</emo>` (character emotion), `<options><o>…</o></options>` (narrator suggestions), `<next-scene/>` (story advance), `<enter>Name</enter>`/`<leave>Name</leave>` (stage presence), `<the-end/>` (playthrough conclusion). Everything fails soft — a malformed tag renders as plain text and the chat keeps going.
