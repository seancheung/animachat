...
2. **Library** → starter characters and places come pre-seeded on a fresh install (Mira, Kael, the Moonlit Tavern — `migrations/100_seed.sql`; delete it before the first `docker compose up` to start empty). Its tabs also hold **Stories** and **Manuscripts**, and one Import / Export flow transfers any combination of content as a bundle. Every editor has an **AI co-writer panel** that fills the form as you chat with it.
3. **Library → Stories** → a story is a self-contained work: its cast, scenes, places and lore live *inside* it (embedded copies — the reusable entity tabs stay a parts bin you copy from). A starter story is seeded; the story page's co-writer can author a whole story in one conversation, or extract one from an attached novel. Hit **Play** to start a playthrough directly from a story.
4. **Chats** → switch between **Casual**, **Immersive**, and **Playthrough**. The New button opens a setup dialog for the active mode; a new playthrough first asks for its story, then shows the existing play settings.
...
| **Entities** | Library tabs: reusable characters (sprites, custom expressions, image prompt), personas, locations & scenes (16:9 art, BGM, ambient loop, image prompt, per-place stage & chat-panel coloring with a global toggle), lorebooks (keyword-triggered), plus self-contained **Stories** and **Manuscripts**. Each story owns embedded copies of its cast, scenes (with per-scene casts, contracts, offstage pressures & authored branching), locations and lorebooks, plus secrets — edited on a full story page with a whole-document co-writer; copy reusable items in or out as snapshots, never live links. One Library import/export flow handles every tab as zip bundles with assets |
