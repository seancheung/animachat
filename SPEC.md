...
1. **Authored branching & multiple endings.** A scene entry may declare **allowed successors** (unset = the next scene in order, exactly today's behavior). Where more than one road is open, the narrator advances with a *targeted* `<next-scene>Scene Name</next-scene>` — the bare `<next-scene/>` keeps meaning "the single/next successor". The payload resolves fail-soft against the allowed successors (like `<enter>` names), and the stage event already stores the target scene id, so folding, forks and backlog replay need nothing new. Each successor may carry an authored **condition hint** ("if trust has grown → Moonlit Confession; if the debt stands → The Collectors' Terms") — guidance for the narrator's judgment, never a mechanical gate, in keeping with the design principle: branches are situations the truths make reachable, not scripted routes. Multiple final scenes are multiple endings — the destination says what "the end" *means*; each final scene is one way of answering it. Fork-at-message is the replay mechanic: forking before a branch point plays the other road, and the "The End" labels of sibling playthroughs become the endings found so far.
2. **Played-character immersion — you are exactly whom you play.** A playthrough is the played character's story, not a seat at the protagonist's; the story never bends itself to be witnessed. The rules:
   - **Start at their entrance, never earlier.** Play opens at the first scene whose authored per-scene cast includes the played character — regardless of the chosen or default starting scene. Everything authored before that point has already happened, offstage: the narrator opens the entrance scene from the played character's own situation, with the world already shaped by those earlier events (playing the princess the prince is to marry, the tale's sea-and-shipwreck opening is over before your first line — you meet the story when it reaches your court).
   - **Only their view, ever.** Scenes whose cast excludes the played character are not playable stops — advancing skips over them to the played character's next scene, and their contracts fold into the world's offstage momentum (see offstage pressures). What happened out of view reaches the played character only as it diegetically would: news, rumors, consequences, arrivals, wreckage.
   - **The view rewrite.** Authored stories usually read from the protagonist's angle; the narrator re-projects them. Premise, contracts and secrets are the truth of the world — not a script of what the player gets to see — and the narrator narrates only the slice within the played character's reach, advancing unseen threads between scenes. The director's dashboard is scoped the same way. An antagonist seat works identically: the story starts at their entrance and advances in their view.
   - **The end is theirs.** If the played character dies or leaves the story for good, the narrator concludes with `<the-end/>` — the earned ending of *this* playthrough, whatever the wider tale would have done. The epilogue register may look outward afterwards. Forking remains the way to try another life.
   Prompt-level rules for the narrator and director plus entrance-scene selection at creation — no new schema; the authored per-scene casts already say where everyone belongs.
3. **Offstage pressures** — an optional per-scene-contract line naming what moves *elsewhere* while the scene plays ("the rival's men search the docks tonight"). The narrator treats them as the world's momentum: they advance between scenes and surface as consequences, so the world stops waiting for the player. Pure prompt material — no counters, no mechanics.
...
- **Malformed field data is diagnosable:** when the held-back fields block fails to parse, the panel's fallback notice ("I produced malformed field data…") is followed by a **Download debug log** link — the server writes the parse error plus the raw model response to a file under `data/logs/` (served by `/api/debug/logs/<file>`, strict filename check). Debug data only, no UI beyond the link. Logging is on by default in development; the `DEBUG_RESPONSE_LOG` env var overrides everywhere and is the only switch in production (`0`/`false`/`off` disable, anything else enables). Downloading is never gated — old links keep working. Log files are never pruned automatically.
