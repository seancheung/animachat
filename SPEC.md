...
- **Organization:** chat tags/folders, auto-generated chat titles, and a chat-list search that filters the list by title, tags, character/persona names, and story name — never message content. The list loads incrementally (infinite scroll); search and the folder filter run server-side. Playthroughs are labeled "Playthrough — <story name>" (from the snapshot, so the label outlives the story) with a "The End" badge once completed.
