...
- **Storage panel:** a settings group shows the asset bucket's object count & total size, with a prune button that deletes objects nothing references (after a confirm showing count & size; unsaved editor uploads count as unused, but anything uploaded within the last hour is left alone so a prune can't yank an upload out from under an open editor — including direct uploads that were never finalized). References counted: characters, locations, scenes, **story documents** (their embedded items), and **playthrough snapshots** — so a finished playthrough keeps its sprites/art/BGM alive even after the story is deleted.
