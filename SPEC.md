...
2. **Played-character staging — the story follows the camera.** The played character is the viewpoint of every turn, so the narrator and director must stage the story within their reach. The rules: events come to the player or the player is plausibly moved to events — never a scene the played character cannot witness; a scene whose per-scene cast wouldn't include them still opens *around* them (their entrance brought forward, or the thread converging on them). If the fiction removes the played character — death, capture, departure — the narrator must either conclude with `<the-end/>` on the ending that exit has earned, or keep a narratable point of view (dying moments, the immediate aftermath, the parallel thread) and steer promptly to a conclusion. A camera left adrift outside the story is a directing failure, not a valid state; forking is the player's escape hatch, never the plan. Prompt-level rules for the narrator and the director's dashboard — no schema.
