...
- **Raw responses are kept for debugging:** every AI chat/narrator message variant stores the model's output verbatim (before tag parsing) alongside the parsed content — inspectable via a collapsible "Raw model output" block in the message editor. Debug data only: never searched, never sent back to models, absent on user messages and on messages saved before the field existed.
