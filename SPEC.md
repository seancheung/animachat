...
All world-building entities are reusable across chats. The library page has a per-type text search (name + tags) and sorting (recently updated / newest / name).
...
- **Organization:** chat tags/folders, auto-generated chat titles, and a chat-list search that filters the list by title, tags, character/persona names, and story name — never message content. Playthroughs are labeled "Playthrough — <story name>" (from the snapshot, so the label outlives the story) with a "The End" badge once completed.
