...
import { ArrowRightLeft, BookOpen, Download, Play, ScrollText, Trash2 } from "lucide-react";
...
export function InteractiveStories() {
  ...
  async function adaptToManuscript(item: any) {
    ...
    try {
      const result = await api.post<{ id: string }>("/api/studio/adapt", { sourceType: "story", id: item.id });
